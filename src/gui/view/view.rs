...
pub(crate) fn build_alice_view() -> impl Widget<AliceModel> {
    ...
    let calc_sign_button = Button::new("Signatur berechnen").on_click(|_ctx, _data: &mut AliceModel, _env| {
...
pub(crate) fn build_bob_view() -> impl Widget<BobModel> {
    ...
    let calc_sign_button = Button::new("Signatur berechnen").on_click(|_ctx, _data: &mut BobModel, _env| {
        _ctx.submit_command(SIGN);
    }).fix_width(fixed_width_button);
