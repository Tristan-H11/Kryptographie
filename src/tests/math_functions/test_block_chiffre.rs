...
mod tests {
    ...
    fn test_create_block_umkehrfunktion_create_string() {
        ...
        let encoded = create_string_from_blocks_encrypt(
            create_blocks_from_string_encrypt(m, block_size, true, 55296),
            block_size + 1,
            55296,
        );
        let decoded = create_string_from_blocks_decrypt(
            create_blocks_from_string_decrypt(&encoded, true, 55296, block_size + 1),
            55296,
        );
    ...
    fn test_create_chiffre() {
        ...
        let block_size = 7;
        let _basis_length = 55296 as u32;
        let result = create_blocks_from_string_encrypt(message, block_size, true, 55296);
