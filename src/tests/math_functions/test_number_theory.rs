#[cfg(test)]
mod tests {
    use crate::encryption::math_functions::number_theory::{
        fast_exponentiation, miller_rabin, modulo_inverse,
    };
    use bigdecimal::num_bigint::{BigInt, BigUint};
    use std::str::FromStr;
    use crate::big_u;

    #[test]
    fn fast_exponentiation_happy_flow() {
        let base = &big_u!(561563u32);
        let exponent = &big_u!(1300u32);
        let modul = &big_u!(564u32);
        assert_eq!(
            fast_exponentiation(base, exponent, modul),
            big_u!(205u32)
        );

        let base = &big_u!(56156334590832345u64);
        let exponent = &big_u!(109458390583094852904812340u128);
        let modul = &big_u!(564234859u32);
        assert_eq!(
            fast_exponentiation(base, exponent, modul),
            big_u!(558376785u32)
        );
    }

    #[test]
    fn fast_exponentiation_exponent_one() {
        let base = &big_u!(561563u32);
        let exponent = &big_u!(1u32);
        let modul = &big_u!(564u32);
        assert_eq!(
            fast_exponentiation(base, exponent, modul),
            big_u!(383u32)
        );
    }

    #[test]
    fn fast_exponentiation_exponent_zero() {
        let base = &big_u!(561563u32);
        let exponent = &big_u!(0u32);
        let modul = &big_u!(564u32);
        assert_eq!(
            fast_exponentiation(base, exponent, modul),
            big_u!(1u32)
        );
    }

    #[test]
    fn fast_exponentiation_base_zero() {
        let base = &big_u!(0u32);
        let exponent = &big_u!(561563u32);
        let modul = &big_u!(564u32);
        assert_eq!(
            fast_exponentiation(base, exponent, modul),
            big_u!(0u32)
        );
    }

    #[test]
    fn fast_exponentiation_modul_one() {
        let base = &big_u!(3459860u32);
        let exponent = &big_u!(561563u32);
        let modul = &big_u!(1u32);
        assert_eq!(
            fast_exponentiation(base, exponent, modul),
            big_u!(0u32)
        );
    }

    #[test]
    fn fast_exponentiation_big_numbers() {
        let base = &big_u!(3459860u32).pow(50);
        let exponent = &big_u!(561563u32).pow(50);
        let modul = &big_u!(345902u32).pow(50);
        assert_eq!(
            fast_exponentiation(base, exponent, modul),
            BigUint::from_str("").unwrap()
        );

        let base = &big_u!(5345890u32).pow(50);
        let exponent = &big_u!(561563u32).pow(50);
        let modul = &big_u!(402u32).pow(453);
        assert_eq!(
            fast_exponentiation(base, exponent, modul),
            BigUint::from_str("").unwrap()
        );

        assert_eq!(
            fast_exponentiation(
                &big_u!(37u32),
                &big_u!(2u32),
                &big_u!(89u32)
            ),
            big_u!(34u32)
        )
    }

    #[test]
    fn modulo_inverse_test() {
        //assert_eq!(modulo_inverse(BigInt::from(1), BigInt::from(3)).unwrap(), BigInt::from(1));
        assert_eq!(
            modulo_inverse(BigInt::from(5), BigInt::from(11)).unwrap(),
            BigInt::from(9)
        );
        assert_eq!(
            modulo_inverse(BigInt::from(315), BigInt::from(661643)).unwrap(),
            BigInt::from(342374)
        );
        assert_eq!(
            modulo_inverse(
                BigInt::from_str("485398853520739824211578869461").unwrap(),
                BigInt::from_str("79617341660363802320192939486040130094939703771377").unwrap()
            )
            .unwrap(),
            BigInt::from_str("7173228757438794445922076835963679049602847038123").unwrap()
        );
        assert!(modulo_inverse(BigInt::from(78), BigInt::from(99)).is_err());
    }

    #[test]
    fn miller_rabin_test() {
        assert_eq!(miller_rabin(&big_u!(11u32), 40), true);
        assert_eq!(
            miller_rabin(&big_u!(8727030382015287123761u128), 40),
            false
        );
        assert_eq!(
            miller_rabin(&big_u!(2459872438590349034582u128), 40),
            false
        );
        assert_eq!(miller_rabin(&big_u!(2211u32), 40), false);
        assert_eq!(
            miller_rabin(
                &BigUint::from_str("79617341660363802320192939486040130094939703771377").unwrap(),
                40
            ),
            true
        );
    }
}
